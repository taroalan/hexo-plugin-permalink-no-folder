...
function myPlugin(hexo) {
  ...
  hexo.extend.filter.register('after_init', function() {
    ...
    const getNewLink = post => {
      ...
      const sourceWithNoPostfix = source.substring(0, source.length - pathFn.extname(source).length);
      ...
      let slugLink = filename.substring(11, filename.length) + '.html';
    ...
    const usePlugin = permalink => config.permalink === ':title.html' && config.permalink_no_folder;
